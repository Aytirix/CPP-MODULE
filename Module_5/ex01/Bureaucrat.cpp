...
int Bureaucrat::signForm(Form &form)
{
	try
	{
		int isSigned = form.beSigned(*this);
		if (isSigned)
		{
			std::cout << RED BOLD "Bureaucrat " RESET BLUE << getName() << GREEN " signed " RESET BLUE << form.getName() << GREEN " successfully!" << RESET << std::endl;
			return true;
		}
		else
		{
			std::cout << RED BOLD "Bureaucrat " RESET BLUE << getName() << RED " failed to sign " RESET BLUE << form.getName() << RED " because it is already signed!" << RESET << std::endl;
			return false;
		}
	}
	catch (const std::exception &e)
	{
		std::cerr << RED BOLD "Bureaucrat " RESET BLUE << getName() << RED " failed to sign " RESET BLUE << form.getName() << RED " because " RESET << e.what() << std::endl;
		return false;
	}
}